...
class StyleToggle extends Component {
  ...
  setupChecked() {
    const originalStyle = this.props.layer.cartocss
    ...
    this.widget.addEventListener('change', (event) => {
      ...
      if (event.detail === true) {
        this.props.layer.style.setContent(this.props.cartocss);
      } else if (event.detail === false) {
        this.props.layer.style.setContent(originalStyle);
      }
    })
  }
    ...
    return (
      <div>
        <span className="as-display--block">
          <p className="as-body"> {this.props.name}</p>
        </span>
        <span className="as-display--block">
